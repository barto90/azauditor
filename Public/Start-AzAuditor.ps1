function Start-AzAuditor {
    ...
    # Use strongly-typed ArrayList to enforce TestResult objects only
            ...
            if (Get-Command $functionName -ErrorAction SilentlyContinue) {
                ...
                if ($testResults) {
                    ...
                    foreach ($r in $testResults) {
                        if ($null -ne $r) {
                            if ($r.ResultStatus -eq 'Pass') { $passCount++ } else { $failCount++ }
                        }
                    }
    ...
    foreach ($result in $allResults) {
        if ($result.ResultStatus -eq 'Pass') { $passCount++ }
        else { $failCount++ }
    }
